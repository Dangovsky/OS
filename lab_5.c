...
int main(int argc, char *argv[]){
	...
	Link foo = createNode("foo", 0, S_IFDIR | 0755); //создаем виртуальную папку foo
	Link bin = createNode("bin", 0, S_IFDIR | 0777); //создаем виртуальную папку bar
	addNode(tree, foo); //Добавляем виртуальную папку foo в виртуальную корневую папку
	addNode(tree, bin);//Добавляем виртуальную папку bar в виртуальную корневую папку
	Link bar = createNode("bar", 0, S_IFDIR | 0777); //создаем виртуальную папку bin
	Link baz = createNode("baz", 0, S_IFDIR | 0444);//создаем виртуальную папку baz
	addNode(bin, bar);//Добавляем виртуальную папку bar в папку bin
	addNode(foo, baz);//Добавляем виртуальную папку baz в папку foo
	Link readme = createNode("readme.txt", "Student Максим Радчук 16150168\r\n", S_IFREG | 0426);//Создаем виртуальный файл readme.txt

	//Получение содержимого /bin/echo
	struct stat echo_stat;//Объявляем переменную echo_stat
	stat("/usr/bin/env", &echo_stat);//Получаем статистику реального файла /usr/bin/env  и записываем в переменную echo_stat
	size_t len = echo_stat.st_size; //Копируем размер файла из echo_stat.st_size в len
	FILE *f;//Объявляем переменную - ссылку на файл
	Link echo = createNode("env", (unsigned char *) malloc(len), S_IFREG | 0767);//Создаем виртуальный файл echo, в качестве содержимого передаем указатель на свободную память с размером len
	f = fopen("/usr/bin/env", "r");//Открываем реальный файл и записываем ссылку на него в f
	...
	addNode(baz, echo);//Добавляем виртуальныйы файл echo виртуальную папку baz
	Link example = createNode("example", "Hello world", S_IFREG | 0277);//Создаем виртуальный файл example
	addNode(baz, example);//Добавляем виртуальный файл example в виртуальную папку baz
	char testtxt_str[68*2] = "";//Создаем массив символов длиной 61*2
	for (int i=0; i<68; i++){//<Любой текст на ваш выбор с количеством строк равным последним двум цифрам номера зачетки>
		strcat(testtxt_str, "1\n");//Записываем в массив символы 1\n (чтобы получилась 68 строк с любым текстом. в данном случае - 68 строка с цифрой 1.
	}
	Link text = createNode("test.txt", testtxt_str, S_IFREG | 0665);//Создаем виртуальный файл test.txt
	addNode(baz, text);//Добавляем виртуальный файл test.txt в виртуальную папку foo
