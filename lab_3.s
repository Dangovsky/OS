.data
    ...
    arg_awk: .string "{split($0, logs, \"\\\"\"); split(logs[2], url, \" \");split(logs[3], data, \" \"); if (data[1] != \"-\"){     sum[url[2]] += data[1]  }} END { PROCINFO[\"sorted_in\"]=\"@val_num_desc\";pos = 1;for(i in sum){	if (pos <= 10) {		total=total+sum[i]        } 	pos++;};pos = 1;for(i in sum){	if (pos <= 10)		{printf(\"%d. %s - %d - %.2f%%\\n\",  pos, i, sum[i], sum[i]/total*100);pos++;}	}}"
